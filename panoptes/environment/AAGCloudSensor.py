...
class AAGCloudSensor(object):
    ...
    def save(self):
        ...
        weather_data = {
            ...
            'data': {
                'Safe': self.safe,
                'Ambient Temperature': self.ambient_temp.value if self.ambient_temp else 'null',
                'Sky Temperature': self.sky_temp.value if self.sky_temp else 'null',
                'Rain Frequency': self.rain_frequency,
                'Wind Speed': self.wind_speed.value if self.wind_speed else 'null',
                'Internal Voltage': self.internal_voltage.value if self.internal_voltage else 'null',
                'LDR Resistance': self.LDR_resistance.value if self.LDR_resistance else 'null',
                'Rain Sensor Temperature': self.rain_sensor_temp.value if self.rain_sensor_temp else 'null',
                'PWM': self.PWM,
                'E1': self.errors['!E1'],
                'E2': self.errors['!E2'],
                'E3': self.errors['!E3'],
