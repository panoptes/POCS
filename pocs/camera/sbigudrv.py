...
class SBIGDriver(PanBase):
    def __init__(self, library_path=False, retries=1, *args, **kwargs):
        ...
        for i in range(self._camera_info.camerasFound):
            self._send_command('CC_OPEN_DRIVER')
            odp = OpenDeviceParams(device_type_codes['DEV_USB{}'.format(i + 1)], 0, 0)
    ...
    def assign_handle(self, serial=None):
        ...
        if serial:
            ...
            serials = [
                str(self._camera_info.usbInfo[i].serialNumber, encoding='ascii')
                for i in range(self._camera_info.camerasFound)
            ]
        ...
        assert camera_serial == ccd_info['serial number'], self.logger.error(
    ...
    def query_temp_status(self, handle):
        ...
        with self._command_lock:
            ...
            self._send_command('CC_QUERY_TEMPERATURE_STATUS', query_temp_params,
                               query_temp_results)
    ...
    def take_exposure(self,
        ...
        start_exposure_params = StartExposureParams2(ccd_codes['CCD_IMAGING'], centiseconds,
                                                     abg_command_code, shutter_command_code,
                                                     readout_mode_code, top, left, height, width)
        ...
        readout_args = (handle, centiseconds, filename, readout_mode_code, top, left, height,
                        width, header, exposure_event)
        readout_thread = Timer(interval=wait, function=self._readout, args=readout_args)
...
# Private methods
    ...
    def _readout(self,
        ...
        start_readout_params = StartReadoutParams(ccd_codes['CCD_IMAGING'], readout_mode_code, top,
                                                  left, height, width)
        ...
        readout_line_params = ReadoutLineParams(ccd_codes['CCD_IMAGING'], readout_mode_code, left,
                                                width)
        ...
        self.logger.debug('Exposure on {} complete'.format(
            self._ccd_info[handle]['serial number']))
    ...
    def _get_ccd_info(self, handle):
        ...
        ccd_info = {
            'firmware version': self._bcd_to_string(ccd_info_results0.firmwareVersion),
    ...
    def _send_command(self, command, params=None, results=None):
        ...
        while error != 'CE_NO_ERROR' and retries_remaining > 0:
            ...
            return_code = self._CDLL.SBIGUnivDrvCommand(command_code, (ctypes.byref(params)
                                                                       if params else None),
...
driver_control_params = {
    i: param
    for i, param in enumerate(('DCP_USB_FIFO_ENABLE', 'DCP_CALL_JOURNAL_ENABLE', 'DCP_IVTOH_RATIO',
                               'DCP_USB_FIFO_SIZE', 'DCP_USB_DRIVER', 'DCP_KAI_RELGAIN',
                               'DCP_USB_PIXEL_DL_ENABLE', 'DCP_HIGH_THROUGHPUT',
                               'DCP_VDD_OPTIMIZED', 'DCP_AUTO_AD_GAIN',
                               'DCP_NO_HCLKS_FOR_INTEGRATION', 'DCP_TDI_MODE_ENABLE',
                               'DCP_VERT_FLUSH_CONTROL_ENABLE', 'DCP_ETHERNET_PIPELINE_ENABLE',
                               'DCP_FAST_LINK', 'DCP_OVERSCAN_ROWSCOLS',
                               'DCP_PIXEL_PIPELINE_ENABLE', 'DCP_COLUMN_REPAIR_ENABLE',
                               'DCP_WARM_PIXEL_REPAIR_ENABLE', 'DCP_WARM_PIXEL_REPAIR_COUNT',
                               'DCP_LAST'))
}
