...
class SBIGDriver(PanBase):
    def __init__(self, library_path=False, *args, **kwargs):
        ...
        if not library_path:
            ...
            if library_path is None:
                self.logger.error(
                    'Could not find SBIG Universal Driver/Library!')
                raise RuntimeError(
                    'Could not find SBIG Universal Driver/Library!')
        ...
        for i in range(self._camera_info.camerasFound):
            self._send_command('CC_OPEN_DRIVER')
            odp = OpenDeviceParams(
                device_type_codes['DEV_USB{}'.format(i + 1)], 0, 0)
    ...
    def assign_handle(self, serial=None):
        ...
        if serial:
            ...
            serials = [
                str(self._camera_info.usbInfo[i].serialNumber,
                    encoding='ascii')
                for i in range(self._camera_info.camerasFound)
            ]
            ...
            if serial not in serials:
                ...
                self.logger.error(
                    'SBIG camera serial number {} not connected!'.format(
                        serial))
            ...
            if self._handle_assigned[index]:
                ...
                self.logger.error(
                    'SBIG camera serial number {} already assigned!'.format(
                        serial))
                return (INVALID_HANDLE_VALUE, None)
        ...
        camera_serial = str(
            self._camera_info.usbInfo[index].serialNumber, encoding='ascii')
    ...
    def set_temp_regulation(self, handle, set_point):
        ...
        set_temp_params = SetTemperatureRegulationParams2(
            enable_code, set_point)

        # Use temperature regulation autofreeze, if available (might marginally reduce read noise).
        autofreeze_code = temperature_regulation_codes[
            'REGULATION_ENABLE_AUTOFREEZE']
        set_freeze_params = SetTemperatureRegulationParams2(
            autofreeze_code, set_point)

        with self._command_lock:
            self._set_handle(handle)
            self._send_command(
                'CC_SET_TEMPERATURE_REGULATION2', params=set_temp_params)
            self._send_command(
                'CC_SET_TEMPERATURE_REGULATION2', params=set_freeze_params)
    ...
    def take_exposure(self,
        ...
        start_exposure_params = StartExposureParams2(
            ccd_codes['CCD_IMAGING'], centiseconds, abg_command_code,
            shutter_command_code, readout_mode_code, top, left, height, width)
        ...
        query_status_params = QueryCommandStatusParams(
            command_codes['CC_START_EXPOSURE2'])
        ...
        if query_status_results.status != status_codes['CS_IDLE']:
            self.logger.warning(
                'Attempt to start exposure on {} while camera busy!'.format(
                    handle))
            # Wait until camera is idle
            while query_status_results.status != status_codes['CS_IDLE']:
                self.logger.warning(
                    'Waiting for exposure on {} to complete'.format(handle))
                time.sleep(1)
                with self._command_lock:
                    self._set_handle(handle)
                    self._send_command(
        ...
        if temp_status.coolingEnabled:
            if abs(temp_status.imagingCCDTemperature - temp_status.ccdSetpoint) > 0.5 or \
               temp_status.imagingCCDPower == 100.0:
                self.logger.warning(
                    'Unstable CCD temperature in {}'.format(handle))
        time_now = Time.now()
        header = fits.Header()
        ...
        header.set('EGAIN', self._ccd_info[handle]['readout_modes'][
            readout_mode]['gain'].value)
        ...
        with self._command_lock:
            self._set_handle(handle)
            self._send_command(
                'CC_START_EXPOSURE2', params=start_exposure_params)

        # Use a Timer to schedule the exposure readout and return a reference to the Timer.
        wait = seconds - 0.1 if seconds > 0.1 else 0.0
        readout_args = (handle, centiseconds, filename, readout_mode_code, top,
                        left, height, width, header, exposure_event)
        readout_thread = Timer(
            interval=wait, function=self._readout, args=readout_args)
...
# Private methods
    ...
    def _get_ccd_info(self, handle):
        ...
        ccd_info_params0 = GetCCDInfoParams(
            ccd_info_request_codes['CCD_INFO_IMAGING'])
        ccd_info_results0 = GetCCDInfoResults0()

        # 'CCD_INFO_EXTENDED' will get bad column info, and whether the CCD has ABG or not.
        ccd_info_params2 = GetCCDInfoParams(
            ccd_info_request_codes['CCD_INFO_EXTENDED'])
        ...
        ccd_info_params4 = GetCCDInfoParams(
            ccd_info_request_codes['CCD_INFO_EXTENDED2_IMAGING'])
        ...
        ccd_info_params6 = GetCCDInfoParams(
            ccd_info_request_codes['CCD_INFO_EXTENDED3'])
        ...
        ccd_info = {
            'firmware_version':
            self._bcd_to_string(ccd_info_results0.firmwareVersion),
            'camera_type':
            camera_types[ccd_info_results0.cameraType],
            'camera_name':
            str(ccd_info_results0.name, encoding='ascii'),
            'bad_columns':
            ccd_info_results2.columns[0:ccd_info_results2.badColumns],
            'imaging_ABG':
            bool(ccd_info_results2.imagingABG),
            'serial_number':
            str(ccd_info_results2.serialNumber, encoding='ascii'),
            'frame_transfer':
            bool(ccd_info_results4.capabilities_b0),
            'electronic_shutter':
            bool(ccd_info_results4.capabilities_b1),
            'remote_guide_head_support':
            bool(ccd_info_results4.capabilities_b2),
            'Biorad_TDI_support':
            bool(ccd_info_results4.capabilities_b3),
            'AO8':
            bool(ccd_info_results4.capabilities_b4),
            'frame_buffer':
            bool(ccd_info_results4.capabilities_b5),
            'dump_extra':
            ccd_info_results4.dumpExtra,
            'STXL':
            bool(ccd_info_results6.camera_b0),
            'mechanical_shutter':
            not bool(ccd_info_results6.camera_b1),
            'colour':
            bool(ccd_info_results6.ccd_b0),
            'Truesense':
            bool(ccd_info_results6.ccd_b1)
        }
    ...
    def _parse_readout_info(self, infos):
        ...
        for info in infos:
            ...
            pixel_height = self._bcd_to_float(
                info.pixelHeight, int_type='ulong')
            readout_mode_info[mode] = {
                'width': info.width * u.pixel,
    ...
    def _disable_vdd_optimized(self, handle):
        ...
        with self._command_lock:
            self._set_handle(handle)
            self._send_command(
                'CC_SET_DRIVER_CONTROL', params=set_driver_control_params)
    ...
    def _send_command(self, command, params=None, results=None):
        ...
        return_code = self._CDLL.SBIGUnivDrvCommand(
            command_code, (ctypes.byref(params)
                           if params else None), (ctypes.byref(results)
        ...
        try:
            ...
            raise RuntimeError(
                "SBIG Driver returned unknown error code '{}'".format(
                    return_code))
        ...
        if error != 'CE_NO_ERROR':
            raise RuntimeError(
                "SBIG Driver returned error '{}'!".format(error))
...
class QueryUSBInfo(ctypes.Structure):
    ...
    _fields_ = [('cameraFound', ctypes.c_ushort),
                ('cameraType', ctypes.c_ushort), ('name', ctypes.c_char * 64),
...
class QueryUSBResults(ctypes.Structure):
    ...
    """
    _fields_ = [('camerasFound', ctypes.c_ushort), ('usbInfo',
                                                    QueryUSBInfo * 4)]
...
class OpenDeviceParams(ctypes.Structure):
    ...
    _fields_ = [('deviceType', ctypes.c_ushort),
                ('lptBaseAddress', ctypes.c_ushort), ('ipAddress',
                                                      ctypes.c_ulong)]
...
class GetLinkStatusResults(ctypes.Structure):
    ...
    """
    _fields_ = [('linkEstablished', ctypes.c_ushort), ('baseAddress',
                                                       ctypes.c_ushort),
                ('cameraType', ctypes.c_ushort), ('comTotal', ctypes.c_ulong),
...
temp_status_requests = {
    0: 'TEMP_STATUS_STANDARD',
    1: 'TEMP_STATUS_ADVANCED',
    2: 'TEMP_STATUS_ADVANCED2'
}

temp_status_request_codes = {
    request: code
    for code, request in temp_status_requests.items()
}
...
class QueryTemperatureStatusResults(ctypes.Structure):
    ...
    """
    _fields_ = [('enabled', ctypes.c_ushort), ('ccdSetpoint', ctypes.c_ushort),
                ('power', ctypes.c_ushort), ('ccdThermistor', ctypes.c_ushort),
...
class QueryTemperatureStatusResults2(ctypes.Structure):
    ...
    """
    _fields_ = [('coolingEnabled',
                 ctypes.c_ushort), ('fanEnabled',
                                    ctypes.c_ushort), ('ccdSetpoint',
                                                       ctypes.c_double),
                ('imagingCCDTemperature',
                 ctypes.c_double), ('trackingCCDTemperature', ctypes.c_double),
                ('externalTrackingCCDTemperature',
                 ctypes.c_double), ('ambientTemperature',
                                    ctypes.c_double), ('imagingCCDPower',
                                                       ctypes.c_double),
                ('trackingCCDPower',
                 ctypes.c_double), ('externalTrackingCCDPower',
                                    ctypes.c_double), ('heatsinkTemperature',
                                                       ctypes.c_double),
                ('fanPower',
                 ctypes.c_double), ('fanSpeed',
                                    ctypes.c_double), ('trackingCCDSetpoint',
                                                       ctypes.c_double)]
...
temperature_regulation_codes = {
    regulation: code
    for code, regulation in temperature_regulations.items()
}
...
class SetTemperatureRegulationParams(ctypes.Structure):
    ...
    """
    _fields_ = [('regulation', ctypes.c_ushort), ('ccdSetpoint',
                                                  ctypes.c_ushort)]
...
ccd_info_request_codes = {
    request: code
    for code, request in ccd_info_requests.items()
}
...
class ReadoutInfo(ctypes.Structure):
    ...
    """
    _fields_ = [('mode', ctypes.c_ushort), ('width', ctypes.c_ushort),
                ('height', ctypes.c_ushort), ('gain', ctypes.c_ushort),
                ('pixelWidth', ctypes.c_ulong), ('pixelHeight',
                                                 ctypes.c_ulong)]
...
class GetCCDInfoResults0(ctypes.Structure):
    ...
    _fields_ = [('firmwareVersion', ctypes.c_ushort),
                ('cameraType', ctypes.c_ushort), ('name', ctypes.c_char * 64),
                ('readoutModes', ctypes.c_ushort), ('readoutInfo',
                                                    ReadoutInfo * 20)]
...
class GetCCDInfoResults2(ctypes.Structure):
    ...
    """
    _fields_ = [('badColumns', ctypes.c_ushort), ('columns',
                                                  ctypes.c_ushort * 4),
                ('imagingABG', ctypes.c_ushort), ('serialNumber',
                                                  ctypes.c_char * 10)]
...
class GetCCDInfoResults4(ctypes.Structure):
    ...
    """
    _fields_ = [('capabilities_b0', ctypes.c_int,
                 1), ('capabilities_b1', ctypes.c_int,
                      1), ('capabilities_b2', ctypes.c_int,
                           1), ('capabilities_b3', ctypes.c_int,
                                1), ('capabilities_b4', ctypes.c_int,
                                     1), ('capabilities_b5', ctypes.c_int, 1),
                ('capabilities_unusued', ctypes.c_int,
                 ctypes.sizeof(ctypes.c_ushort) * 8 - 6), ('dumpExtra',
                                                           ctypes.c_ushort)]
...
class GetCCDInfoResults6(ctypes.Structure):
    ...
    """
    _fields_ = [('camera_b0', ctypes.c_int, 1), ('camera_b1', ctypes.c_int, 1),
                ('camera_unused', ctypes.c_int,
                 30), ('ccd_b0', ctypes.c_int, 1), ('ccd_b1', ctypes.c_int, 1),
                ('ccd_unused', ctypes.c_int, 30), ('extraBits', ctypes.c_int,
                                                   32)]
...
driver_control_params = {
    i: param
    for i, param in enumerate((
        'DCP_USB_FIFO_ENABLE', 'DCP_CALL_JOURNAL_ENABLE', 'DCP_IVTOH_RATIO',
        'DCP_USB_FIFO_SIZE', 'DCP_USB_DRIVER', 'DCP_KAI_RELGAIN',
        'DCP_USB_PIXEL_DL_ENABLE', 'DCP_HIGH_THROUGHPUT', 'DCP_VDD_OPTIMIZED',
        'DCP_AUTO_AD_GAIN', 'DCP_NO_HCLKS_FOR_INTEGRATION',
        'DCP_TDI_MODE_ENABLE', 'DCP_VERT_FLUSH_CONTROL_ENABLE',
        'DCP_ETHERNET_PIPELINE_ENABLE', 'DCP_FAST_LINK',
        'DCP_OVERSCAN_ROWSCOLS', 'DCP_PIXEL_PIPELINE_ENABLE',
        'DCP_COLUMN_REPAIR_ENABLE', 'DCP_WARM_PIXEL_REPAIR_ENABLE',
        'DCP_WARM_PIXEL_REPAIR_COUNT', 'DCP_LAST'))
}

driver_control_codes = {
    param: code
    for code, param in driver_control_params.items()
}
...
class GetDriverControlParams(ctypes.Structure):
    ...
    """
    _fields_ = [
        ('controlParameter', ctypes.c_ushort),
    ]
...
class SetDriverControlParams(ctypes.Structure):
    ...
    """
    _fields_ = [('controlParameter', ctypes.c_ushort), ('controlValue',
                                                        ctypes.c_ulong)]
...
class StartExposureParams2(ctypes.Structure):
    ...
    """
    _fields_ = [('ccd', ctypes.c_ushort), ('exposureTime', ctypes.c_ulong),
                ('abgState', ctypes.c_ushort), ('openShutter',
                                                ctypes.c_ushort),
                ('readoutMode',
                 ctypes.c_ushort), ('top', ctypes.c_ushort), ('left',
                                                              ctypes.c_ushort),
                ('height', ctypes.c_ushort), ('width', ctypes.c_ushort)]


# CCD selection for cameras with built in or connected tracking CCDs
ccds = {0: 'CCD_IMAGING', 1: 'CCD_TRACKING', 2: 'CCD_EXT_TRACKING'}
...
shutter_command_codes = {
    command: code
    for code, command in shutter_commands.items()
}
...
class StartReadoutParams(ctypes.Structure):
    ...
    """
    _fields_ = [('ccd', ctypes.c_ushort), ('readoutMode', ctypes.c_ushort),
                ('top', ctypes.c_ushort), ('left', ctypes.c_ushort),
                ('height', ctypes.c_ushort), ('width', ctypes.c_ushort)]
...
class ReadoutLineParams(ctypes.Structure):
    ...
    """
    _fields_ = [('ccd', ctypes.c_ushort), ('readoutMode', ctypes.c_ushort),
                ('pixelStart', ctypes.c_ushort), ('pixelLength',
                                                  ctypes.c_ushort)]
...
# Reverse dictionary
driver_request_codes = {
    request: code
    for code, request in driver_requests.items()
}
...
class GetDriverInfoResults0(ctypes.Structure):
    ...
    """
    _fields_ = [('version', ctypes.c_ushort), ('name', ctypes.c_char * 64),
