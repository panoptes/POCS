...
class AbstractFocuser(PanBase):
    ...
    def __init__(self,
        ...
        if autofocus_range:
            self.autofocus_range = (int(autofocus_range[0]),
                                    int(autofocus_range[1]))
        else:
            self.autofocus_range = None

        if autofocus_step:
            self.autofocus_step = (int(autofocus_step[0]),
                                   int(autofocus_step[1]))
        else:
        ...
        self.logger.debug('Focuser created: {} on {}'.format(
            self.name, self.port))
...
##################################################################################################
    ...
    def camera(self, camera):
        if self._camera:
            self.logger.warning(
                "{} already assigned to camera {}, skipping attempted assignment to {}!".
                format(self, self.camera, camera))
        else:
            self._camera = camera
    ...
    def autofocus(self,
        ...
        if not focus_range:
            if self.autofocus_range:
                ...
                raise ValueError(
                    "No focus_range specified, aborting autofocus of {}!".
                    format(self._camera))
        ...
        if not focus_step:
            if self.autofocus_step:
                ...
                raise ValueError(
                    "No focus_step specified, aborting autofocus of {}!".
                    format(self._camera))
        ...
        if not seconds:
            if self.autofocus_seconds:
                ...
                raise ValueError(
                    "No focus exposure time specified, aborting autofocus of {}!".
                    format(self._camera))
        ...
        if not thumbnail_size:
            if self.autofocus_size:
                ...
                raise ValueError(
                    "No focus thumbnail size specified, aborting autofocus of {}!".
                    format(self._camera))
    ...
    def _autofocus(self, seconds, focus_range, focus_step, thumbnail_size,
        ...
        if imax == 0 or imax == (n_positions - 1):
            ...
            fit = models.Lorentz1D(
                x_0=focus_positions[imax], amplitude=metric.max())
