...
def test_create_observation_exp_time_no_units(field):
    with pytest.raises(TypeError):
        Observation(field, exposure=1.0)


def test_create_observation_exp_time_bad(field):
    with pytest.raises(AssertionError):
        Observation(field, exposure=0.0 * u.second)


def test_create_observation_exp_time_minutes(field):
    obs = Observation(field, exposure=5.0 * u.minute)
    assert obs.exposure == 300 * u.second
...
def test_small_sets(field):
    obs = Observation(field, exposure=1 * u.second, min_nexp=1, exp_set_size=1)
...
def test_print(field):
    obs = Observation(field, exposure=17.5 * u.second, min_nexp=27, exp_set_size=9)
