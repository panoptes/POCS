...
def test_logger_no_output(caplog, profile, tmp_path):
    ...
    logger = get_logger(log_file='testing.log',
...
def test_base_logger(caplog, profile, tmp_path):
    logger = get_logger(log_dir=str(tmp_path), profile=profile, stderr=True)
    logger.debug('Hello')
...
def test_root_logger(caplog, profile, tmp_path):
    ...
    logger = get_logger(log_file='foo.log', profile=profile, stderr=True)
    ...
    logger = get_logger(profile=profile, stderr=True)
