...
def test_normal():
    hp = Horizon(obstructions=[[[20, 10], [40, 70]]])
    assert isinstance(hp, Horizon)

    hp2 = Horizon(obstructions=[[[40, 45], [50, 50], [60, 45]]])
...
def test_bad_length_tuple():
    with pytest.raises(AssertionError):
        Horizon(obstructions=[[[20], [40, 70]]])


def test_bad_length_list():
    with pytest.raises(AssertionError):
        Horizon(obstructions=[[[40, 70]]])


def test_bad_string():
    with pytest.raises(AssertionError):
        Horizon(obstructions=[[["x", 10], [40, 70]]])
...
def test_negative_alt():
    with pytest.raises(AssertionError):
        Horizon(obstructions=[[[10, 20], [-1, 30]]])


def test_good_negative_az():
    hp = Horizon(obstructions=[[[50, -10], [45, -20]]])
    assert isinstance(hp, Horizon)

    hp2 = Horizon(obstructions=[[[10, -181], [20, -190]]])
    assert isinstance(hp2, Horizon)


def test_bad_negative_az():
    with pytest.raises(AssertionError):
        Horizon(obstructions=[[[10, -361], [20, -350]]])
...
def test_sorting():
    ...
    assert hp.obstructions == [[(10.0, 10.0), (20.0, 20.0)], [(10.0, 50.0), (30.0, 60.0)],
