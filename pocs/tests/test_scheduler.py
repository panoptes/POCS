...
def test_bad_scheduler_type(config):
    ...
    with pytest.raises(error.NotFound):
        create_scheduler_from_config(conf, observer=details['observer'])
...
def test_no_scheduler_in_config(config):
    conf = config.copy()
    del conf['scheduler']
    details = setup_site_location_details_from_config(conf)
    assert create_scheduler_from_config(conf, observer=details['observer']) is None
