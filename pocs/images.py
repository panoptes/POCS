...
class Image(PanBase):
    ...
    def __init__(self, fits_file, wcs_file=None, location=None):
        ...
        assert fits_file.lower().endswith(
            ('.fits')), self.logger.warning('File must end with .fits')
    ...
    def wcs_file(self, filename):
        if filename is not None:
            try:
                ...
                self.logger.debug(
                    "Can't get WCS from FITS file (try solve_field)")
    ...
    def pointing_error(self):
        ...
        if self._pointing_error is None:
            assert self.pointing is not None, self.logger.warn(
    ...
    def get_header_pointing(self):
        ...
        try:
            ...
            self.logger.warning(
                'Cannot get header pointing information: {}'.format(e))
    ...
    def get_wcs_pointing(self):
        ...
        if self.wcs is not None:
            ...
            # Precess to the current equinox otherwise the RA - LST method will
            # be off.
            self.ha = self.pointing.transform_to(
                self.FK5_Jnow).ra.to(
    ...
    def compute_offset(self, ref_image):
        ...
        return OffsetError(d_ra.to(u.arcsec), d_dec.to(
            u.arcsec), mag.to(u.arcsec))
