...
class PanMongo(object):
    ...
    def __init__(self, db='panoptes', host='localhost',
                 port=27017, connect=False, *args, **kwargs):
    ...
    def export(self,
        ...
        for collection in collections:
            ...
            try:
                ...
                entries = [x for x in col.find(
                    {'date': {'$gt': start, '$lt': end}})]

            if len(entries):
                console.color_print(
                    "\t\t{} records exported".format(
                        len(entries)), 'yellow')
                ...
                with open(out_file, write_type)as f:
                    console.color_print(
                        "\t\tWriting file: ", 'lightblue', out_file, 'yellow')
                    f.write(content)
