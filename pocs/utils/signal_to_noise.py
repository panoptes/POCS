...
class Optic:
    ...
    def __init__(self, aperture, focal_length, throughput_filename, central_obstruction=0 * u.mm):
        ...
        Args:
            ...
            central_obstruction: Central obstruction of the Optic
        """
...
class Camera:
    ...
    def __init__(self, bit_depth, full_well, gain, readout_time, pixel_size, resolution, read_noise, dark_current,
        ...
        Args:
            bit_depth: bits per pixel of the camera image data
            full_well: amount of charge an individual pixel can hold before saturating
            gain: amplification that the CCD can provide
            readout_time: readout time between exposures taken by the camera
            pixel_size: size of each pixel
            resolution: number of pixels in the CCD
            read_noise: amount of noise generated by electronics as the charge in the pixels is transferred to the camera
            dark_current: the residual electric current flowing in the camera when there is no incident illumination
            QE_filename: data file containing information regarding Quantum Efficiency versus Wavelength
            minimum_exposure: minimum exposure time that the camera can take
        """
...
class Filter:
    ...
    def __init__(self, transmission_filename, sky_mu):
        """Class representing a bandpass filter

        Args:
            transmission_filename: data file containing information about the transmission rate across a range of wavelengths
            in the filter
            sky_mu: the sky surface brightness for the filter band
        """
...
class Imager:
    ...
    def __init__(self, optic, camera, band, num_imagers=1, num_per_computer=1, PSF=None):
        """
        Class representing an imager

        Args:
            optic: An instance of the Optic class
            camera: An instance of the Camera class
            band: An instance of the Bandpass filter class
            num_imagers: num of imagers with a default value of 1
            num_per_computer: num of imagers connected to a single computer, defaulted to a value of 1
            PSF: An instance of the Moffat_PSF class
        """
    ...
    def SB_signal_noise(self, signal_SB, total_exp_time, sub_exp_time=300 * u.second, binning=1):
        """Calculates the signal and noise
        ...
        if (total_exp_time != number_subs * sub_exp_time):
            ...
            warn('Rounding up total exposure to next integer multiple of sub-exposure time: {}'.format(total_exp_time),
...
class ImagerArray:
    ...
    def __init__(self, imagers):
        ...
        Args:
            imagers: A python dictionary with instances of imager class as values to the keys
        """
    ...
    def HDR_mode(self, minimum_magnitude, name, long_exposures=1, factor=2, maximum_exptime=300 * u.second,
        ...
        Args:
            ...
            maximum_exptime: Maximum possible exposure time we can assign to the imagers
    ...
    def exposure_time_array(self, minimum_magnitude, name, long_exposures=1, factor=2, maximum_exptime=300 * u.second,
        ...
        '''Creating a list of exposure times from the lowest exposure time to the one directly below the
        maximum exposure time'''
        exp_list = [shortest_exposure.to(u.second).value * factor**i for i in range(0, len_exp_array)]

        '''Since we are using both long_exposures and maximum_magnitude as our parameters, we want a way to ensure that both
        parameters aren't being passed by the user. If they are, a ValueError is raised. If none are passed, a default value
        of 1 is passed to long_exposures'''
