...
##
    def append_cands(self, attribs):
        ...
        priority = self.get_priority(attribs['type'])
        expiery = self.get_expiration(attribs['type'])
        exposure = self.get_exposure(attribs['type'])
...
##
    def is_target_available(self, vo):
        ...
        if is_parsed == False:
            ...
            if 'LVC' in attribs['name'] or 'LIGO' in attribs['name']:
            ...
            elif self.is_trusted(attribs['channel'], attribs['author']) == True:
...
if __name__ == '__main__':
    ...
    alerter.alert_pocs(is_target_available, citation)
