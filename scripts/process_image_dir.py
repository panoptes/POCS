...
def main(directory, upload=True, send_headers=True, verbose=False, **kwargs):
    ...
    try:
        clean_observation_dir(directory, remove_jpgs=False, verbose=verbose, **kwargs)
    ...
    if upload:
        ...
        if os.path.exists(uploaded_files_fn):
            ...
            img_path = upload_observation_to_bucket(
                pan_id,
                directory,
                include_files='*',
                verbose=verbose, **kwargs)
            uploaded_files = sorted(glob(img_path))
