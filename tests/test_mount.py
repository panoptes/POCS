...
def test_create_mount_simulator_with_config():
    ...
    set_config("simulator", hardware.get_all_names(without=["mount"]), persist=False)
    # But setting the driver to `simulator` should return simulator.
    set_config("mount.driver", "panoptes.pocs.mount.simulator", persist=False)
...
def test_create_mount_without_mount_info():
    ...
    set_config("mount", None, persist=False)
    set_config("simulator", hardware.get_all_names(without=["mount"]), persist=False)
...
def test_create_mount_with_earth_location():
    ...
    set_config("location", None, persist=False)
    set_config("simulator", hardware.get_all_names(), persist=False)
...
def test_bad_mount_port():
    ...
    simulators = get_config("simulator", default=[])
    ...
    set_config("simulator", simulators, persist=False)
    # Use real mount driver.
    set_config("mount.driver", "panoptes.pocs.mount.ioptron.cem40", persist=False)

    # Set a bad port, which should cause a fail before actual mount creation.
    set_config("mount.serial.port", "foobar", persist=False)
    with pytest.raises(error.MountNotFound):
...
def test_bad_mount_driver():
    ...
    set_config("mount.driver", "not.a.real.mount", persist=False)
    with pytest.raises(error.MountNotFound):
        create_mount_from_config()
    reset_conf()
