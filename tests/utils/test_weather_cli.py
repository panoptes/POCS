...
def test_setup_finds_aag_on_first_port(mock_glob, mock_serial_cls, mock_comports, mock_run, cli_runner):
    ...
    with patch("panoptes.pocs.config_store.set_config"):
...
def test_setup_skips_mount_port(mock_glob, mock_serial_cls, mock_comports, mock_run, cli_runner):
    ...
    with (
        ...
        patch("panoptes.pocs.config_store.set_config"),
    ):
...
def test_setup_config_update_fails(mock_glob, mock_serial_cls, mock_comports, mock_run, cli_runner):
    ...
    with patch("panoptes.pocs.config_store.set_config", side_effect=RuntimeError("write failed")):
        result = cli_runner.invoke(app, ["weather", "setup"])
