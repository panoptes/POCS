...
def test_small_sets(config_port, field):
...
def test_seq_time(config_port, field):
    obs = Observation(field, exptime=17.5 * u.second, min_nexp=27,
                      exp_set_size=9, config_port=config_port)
    assert obs.seq_time is None


def test_no_exposures(config_port, field):
    obs = Observation(field, exptime=17.5 * u.second, min_nexp=27,
                      exp_set_size=9, config_port=config_port)
...
def test_last_exposure_and_reset(config_port, field):
    obs = Observation(field, exptime=17.5 * u.second, min_nexp=27,
                      exp_set_size=9, config_port=config_port)
    status = obs.status
