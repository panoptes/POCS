...
class POCS(Machine, PanBase):
    ...
    def __init__(self, observatory, state_machine_file=None, simulators=None, *args, **kwargs):
        ...
        if "states" not in state_machine_table:
            msg = "State machine configuration is missing required 'states' section."
            self.logger.error(msg)
            raise error.InvalidConfig(msg)

        if "transitions" not in state_machine_table:
            msg = "State machine configuration is missing required 'transitions' section."
            self.logger.error(msg)
            raise error.InvalidConfig(msg)
    ...
    def check_safety(self, event_data=None):
        ...
        if event_data is None:
            self.logger.debug("Checking safety with no event data")
            return self.is_safe()
