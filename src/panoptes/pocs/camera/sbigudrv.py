...
class SBIGDriver(AbstractSDKDriver):
    ...
    def get_SDK_version(self, request_type="DRIVER_STD"):
        ...
        version_string = "{}, {}".format(
            driver_info_results.name.decode("ascii"),
    ...
    def get_link_status(self):
        ...
        link_status = {
            "established": bool(lsr.linkEstablished),
    ...
    def query_temp_status(self, handle):
        ...
        temp_status = {
            "cooling_enabled": bool(qtr.coolingEnabled),
    ...
    def get_exposure_status(self, handle):
        ...
        with self._command_lock:
            self.set_handle(handle)
            self._send_command(
                "CC_QUERY_COMMAND_STATUS", params=query_status_params, results=query_status_results
            )
    ...
    def readout(self, handle, readout_mode, top, left, height, width):
        ...
        start_readout_params = StartReadoutParams(
    ...
    def cfw_init(self, handle, model="AUTO", timeout=10 * u.second):
        ...
        poll_thread = threading.Thread(
            target=self._cfw_poll, args=(handle, 1, model, init_event, timeout), daemon=True
        )
        poll_thread.start()
        init_event.wait()
    ...
    def cfw_goto(self, handle, position, model="AUTO", cfw_event=None, timeout=10 * u.second):
        ...
        if position < 1 or position > info["n_positions"]:
            msg = "Position must be between 1 and {}, got {}".format(info["n_positions"], position)
        ...
        poll_thread = threading.Thread(
            target=self._cfw_poll, args=(handle, position, model, cfw_event, timeout), daemon=True
        )
        poll_thread.start()
    ...
    def _cfw_parse_results(self, cfw_results):
        ...
        """
        results = {
            "model": CFWModelSelect(cfw_results.cfwModel).name,
...
class QueryUSBResults(ctypes.Structure):
    ...
    _fields_ = [("camerasFound", ctypes.c_ushort), ("usbInfo", QueryUSBInfo * 4)]
...
class QueryUSBResults2(ctypes.Structure):
    ...
    _fields_ = [("camerasFound", ctypes.c_ushort), ("usbInfo", QueryUSBInfo * 8)]
...
class QueryUSBResults3(ctypes.Structure):
    ...
    _fields_ = [("camerasFound", ctypes.c_ushort), ("usbInfo", QueryUSBInfo * 24)]
...
class GetLinkStatusResults(ctypes.Structure):
    ...
    _fields_ = [
        ("linkEstablished", ctypes.c_ushort),
...
class SetTemperatureRegulationParams(ctypes.Structure):
    ...
    _fields_ = [("regulation", ctypes.c_ushort), ("ccdSetpoint", ctypes.c_ushort)]
...
class SetTemperatureRegulationParams2(ctypes.Structure):
    ...
    _fields_ = [("regulation", ctypes.c_ushort), ("ccdSetpoint", ctypes.c_double)]
...
class GetCCDInfoResults4(ctypes.Structure):
    ...
    _fields_ = [
        ("capabilities_b0", ctypes.c_int, 1),
...
class SetDriverControlParams(ctypes.Structure):
    ...
    _fields_ = [("controlParameter", ctypes.c_ushort), ("controlValue", ctypes.c_ulong)]
...
abg_states = {0: "ABG_LOW7", 1: "ABG_CLK_LOW7", 2: "ABG_CLK_MED7", 3: "ABG_CLK_HI7"}
...
statuses = {0: "CS_IDLE", 1: "CS_IN_PROGRESS", 2: "CS_INTEGRATING", 3: "CS_INTEGRATION_COMPLETE"}
...
driver_requests = {0: "DRIVER_STD", 1: "DRIVER_EXTENDED", 2: "DRIVER_USB_LOADER"}
